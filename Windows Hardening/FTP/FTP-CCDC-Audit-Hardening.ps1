...
foreach ($site in $ftpSites) {
    ...
    try {
        ...
        if ($ApplyChanges) {
            if ($anonEnabled -eq $true) {
                Set-WebConfigurationProperty -PSPath $psPath -Filter $anonFilter -Name enabled -Value $false
                Write-Report "  Changed: Anonymous Authentication disabled"
            }
...
foreach ($site in $ftpSites) {
    ...
    try {
        ...
        if ($controlPolicy -eq "SslAllow" -or $dataPolicy -eq "SslAllow") {
            Write-Report "  WARNING: SSL is allowed but not required."
        }
    } catch {
